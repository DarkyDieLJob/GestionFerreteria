...
class TestStaffResetRequestViews:
    ...
    def setup(self):
        ...
        self.detail_url = reverse(
            "core_auth:staff_reset_request_detail", args=[self.prr.id]
        )
        self.approve_url = reverse(
            "core_auth:staff_reset_request_approve", args=[self.prr.id]
        )
    ...
    def test_approve_requires_staff(self, client):
        ...
        assert (reverse("core_auth:login") in resp.url) or (
            reverse("admin:login") in resp.url
        )
...
class TestForcePasswordChangeMiddleware:
    ...
    def test_password_change_invalid_old_password_keeps_flag(self, client):
        ...
        assert (
            "incorrecta" in resp.content.decode()
            or "invalid" in resp.content.decode().lower()
        )
    ...
    def test_password_change_mismatch_new_passwords_keeps_flag(self, client):
        ...
        assert profile.must_change_password is True
        assert (
            "coinciden" in resp.content.decode()
            or "match" in resp.content.decode().lower()
        )
    ...
    def test_unresolvable_path_does_not_crash_and_no_redirect_when_flag_false(
        self, client
    ):
