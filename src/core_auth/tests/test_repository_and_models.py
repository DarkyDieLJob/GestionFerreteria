...
from core_auth.adapters.repository import (
    DjangoAuthRepository,
    DjangoCore_authRepository,
)
...
class TestRepository:
    ...
    def test_create_user_duplicate_email_raises_value_error(self, monkeypatch):
        ...
        monkeypatch.setattr(
            self.User.objects, "create_user", lambda **kw: fake_create_user(**kw)
        )
        with pytest.raises(ValueError):
    ...
    def test_create_user_other_integrity_error_maps_to_value_error(self, monkeypatch):
        ...
        User = self.User
        monkeypatch.setattr(
            User.objects, "create_user", lambda **kw: fake_create_user(**kw)
        )
        with pytest.raises(ValueError):
...
def test_models_str_methods():
    ...
    prr = PasswordResetRequest.objects.create(
        identifier_submitted="idX", status="pending"
    )
    s = str(prr)
