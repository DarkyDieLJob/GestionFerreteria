...
class CoreAuthProfile(models.Model):
    ...
    user = models.OneToOneField(
        get_user_model(), on_delete=models.CASCADE, related_name="core_profile"
    )
    ...
    recovery_hint = models.CharField(
        max_length=64, blank=True, default="Últimos 4 del DNI"
    )
...
class PasswordResetRequest(models.Model):
    ...
    dni_last4_provided = models.CharField(
        max_length=4, blank=True, default=""
    )  # no almacenar si no es necesario, ideal comparar y descartar
    ...
    short_code = models.CharField(
        max_length=16, unique=True, editable=False, default=""
    )
    ...
    processed_at = models.DateTimeField(
        null=True, blank=True, help_text="Cuando se generó la temporal"
    )
    delivered_at = models.DateTimeField(
        null=True, blank=True, help_text="Cuando se activó y entregó al usuario"
    )
    ...
    def __str__(self) -> str:
        ...
        base = (
            (self.identifier_submitted or "").strip()
            or self.username_input
            or self.email_input
            or "unknown"
        )
