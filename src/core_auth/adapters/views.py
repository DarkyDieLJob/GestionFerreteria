        ...
        if form.is_valid():
            try:
                ...
                if User.objects.filter(
                    ...
                    return render(
                        request, self.template_name, {"form": form}, status=200
                    )
                ...
                messages.success(
                    request, _("¡Registro exitoso! Por favor, inicia sesión.")
                )
...
class StaffRequiredMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(
            self.request, _("No tienes permisos para acceder a esta sección.")
        )
...
class ResetRequestListView(StaffRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if scope == "all":
            qs = PasswordResetRequest.objects.select_related(
                "user__core_profile"
            ).order_by("-created_at")
        else:
            ...
            qs = (
                PasswordResetRequest.objects.select_related("user__core_profile")
                .filter(
                    Q(status="pending")
                    | Q(
                        status="approved", user__core_profile__must_change_password=True
                    )
                )
                .order_by("-created_at")
            )
...
def approve_reset_request(request, pk):
    ...
    def _inner(req, pk):
        ...
        temp_password = _generate_temp_password(
            getattr(settings, "TEMP_PASSWORD_LENGTH", 16)
        )
...
def deliver_reset_request(request, pk):
    ...
    def _inner(req, pk):
        ...
        messages.success(
            req,
            _("Contraseña temporal activada y lista para ser comunicada al usuario."),
        )
        return redirect(reverse("core_auth:staff_reset_request_detail", args=[pk]))
...
class LoginView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            missing_field_errors = ("username" in form.errors) or (
        ...
        try:
            ...
            try:
                ...
                def send_error(msg):
                    if hasattr(request, "_messages") and hasattr(
            ...
            logger.error(
                f"Error inesperado durante el inicio de sesión: {str(e)}", exc_info=True
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            try:
                ...
                print(e)
                messages.error(request, _("Ocurrió un error al cerrar la sesión."))
